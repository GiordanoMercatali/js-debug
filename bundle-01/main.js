...
function addIfEven(num) {
    if (num % 2 === 0) { // We need "===" to compare the two values, since "=" is used to set the first equal to the second.
        num += 5; // The function will return "num" without the +5
    }
    console.log("Result of ex.2: " + num);
    return num;
}
let number = 4;
addIfEven(number);
...
function displayEvenNumbers() {
    ...
    for (let i = 0; i < numbers.length; i++) { // The last condition of the for loop must not have the "semicolon", also if we want the "evenNumber" array to contain 8, we don't need the - 1 after numbers.length.
        ...
        if (curNumber % 2 === 0); { // We don't compare the entire array, just the value in the "i" position, so I made a curNumber element to contain that value. We need "===" to compare the two values, since "=" is used to set the first equal to the second.
            evenNumbers.push(numbers[i]); //We don't need "i", but the current element of "numbers" array.
        }
